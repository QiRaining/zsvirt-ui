...
@InputType()
class AddNicPayload extends IpConfigPayload {
  @Field(() => String)
  uuid: string
...
@InputType()
export class AddNicInput {
  @Field(() => [AddNicPayload])
  payload: AddNicPayload[]

  @Field(() => ActionInput)
  action: ActionInput
}

export class AddNicService extends ActionService {
  @Inject() updateAction: UpdateBondingAction
  @Inject() getHostNetworkFactsAction: GetHostNetworkFactsAction

  @Mutation(() => ActionResult)
  addNic(@Args('input') input: AddNicInput) {
    const actionId = input.action.actionId
    this.actionHelper(input, 'Bond', async (payload: AddNicPayload, taskId: string) => {
