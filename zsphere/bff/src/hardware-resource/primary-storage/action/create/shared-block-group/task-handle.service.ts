...
import { PrimaryStorageVO } from '../../../primary-storage.model'
import { CreateSharedBlockGroupPrimaryStorageTaskService } from './create-shared-block-group-task.service'

@Injectable()
export class CreateSharedBlockGroupTaskHandlerService {
  ...
  async action(task) {
    ...
    // 找到主任务
    const mainTaskResult = task.children[0].children.find(
      task => task.service === CreateSharedBlockGroupPrimaryStorageTaskService.name
    ).result

    let payload
    if (task.state === FlowConst.state.FINISHED) {
      await this.recordActionService.recordTaskSuccess(task.config.info.taskId)
      const inventory: PrimaryStorageVO = mainTaskResult.inventory
      payload = {
