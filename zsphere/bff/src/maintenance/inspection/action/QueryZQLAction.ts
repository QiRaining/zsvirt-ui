...
@Injectable()
export class QueryZQLAction extends QueryBase {
  @Inject() zopsHttpService: ZopsHttpService
  @Inject() declare webhookCallbackService: WebhookCallbackService

  async call(param: QueryZQLActionParam, _info: ActionInfo = {}): Promise<QueryZQLResult> {
...
export interface QueryZQLActionParam {
  zql: string
}

export interface QueryZQLResult {
  data?: any
}
