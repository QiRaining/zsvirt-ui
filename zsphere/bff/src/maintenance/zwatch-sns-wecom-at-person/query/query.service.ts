...
@Injectable()
export class SNSWeComAtPersonQueryService {
  ...
  async queryList(param: IQueryAction) {
    ...
    _resultResp = await this.getSNSWeComAtPersons(param, zqlCondition)
  ...
  async getSNSWeComAtPersons(param: IQueryAction, zqlCondition: ZqlObject['condition']) {
    const zqlObject = {
