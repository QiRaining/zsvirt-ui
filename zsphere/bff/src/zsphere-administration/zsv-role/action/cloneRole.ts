...
export class CloneRoleService extends ActionService {
  @Inject() createRoleAction: CreateRoleAction
  @InjectModel(ZsRolePrivilege) private zsRolePrivilege: typeof ZsRolePrivilege

  @Mutation(() => ActionResult)
  cloneRole(@Args('input') input: CloneRoleInput) {
    const actionId = input.action.actionId

    const actionFn = async (payload: CloneRolePayload, taskId: string) => {
