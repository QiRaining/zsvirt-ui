...
type IKey = 'name' | 'snmpAddress' | 'snmpPort' | 'uuid'
...
const useQueryConfig = (
  ...
  const remoteConfig = useRemoteConfig('snmp-trap')
  ...
  const originCandidates = useMemo(() => {
    const list: ICandidate[] = [
      ...
      {
        label: intl.formatMessage({ id: 'snmp.trap.ipAddress', defaultMessage: 'IP Address' }),
        key: 'snmpAddress',
        type: 'input'
      },
      {
        label: intl.formatMessage({ id: 'snmp.trap.port', defaultMessage: 'Port' }),
        key: 'snmpPort',
        type: 'input'
      },
      {
