...
type IKey = 'name' | 'uuid' | 'ip' | 'gateway' | 'netmask.prefixlen' | 'owner'
...
const useQueryConfig = (
  ...
  const remoteConfig = useRemoteConfig('vip')
  ...
  const originCandidates = useMemo(() => {
    const list: ICandidate[] = [
      ...
      {
        label: intl.formatMessage({ id: 'ip.address', defaultMessage: 'IP Address' }),
        key: 'ip',
        type: 'input'
      },
      {
        label: intl.formatMessage({ id: 'gateway', defaultMessage: 'Gateway' }),
        key: 'gateway',
        type: 'input'
      },
      {
        label: intl.formatMessage({ id: 'netmask.prefix.length', defaultMessage: 'Netmask/Prefix Length' }),
