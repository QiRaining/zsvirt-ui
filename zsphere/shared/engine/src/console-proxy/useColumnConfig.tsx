...
type IKey = 'managementIp' | 'consoleProxyOverriddenIp' | 'type' | 'state' | 'status' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'console.proxy.overriddenIp', defaultMessage: 'Console Proxy Address' }),
        i18nKey: 'console.proxy.overriddenIp',
        key: 'consoleProxyOverriddenIp',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('consoleProxyOverriddenIp', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('consoleProxyOverriddenIp', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['managementIp', 'consoleProxyOverriddenIp', 'state', 'status', 'createDate'],
      'select': [],
      'sub': [],
      'sub.vm': [],
    }
