...
type IKey = 'name' | 'uuid' | 'type' | 'vtepNum' | 'physicalInterface' | 'vlan' | 'shareType' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'sdn.type', defaultMessage: 'SDN Type ' }),
        i18nKey: 'sdn.type',
        key: 'type',
        width: 200,filters: [],
        render: (value: any) => {
      ...
      {
        title: intl.formatMessage({ id: 'vtep.num', defaultMessage: 'VTEPs' }),
        i18nKey: 'vtep.num',
        key: 'vtepNum',
        width: 100,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('vtepNum', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('vtepNum', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
      ...
      {
        title: intl.formatMessage({ id: 'vlan', defaultMessage: 'VLAN' }),
        i18nKey: 'vlan',
        key: 'vlan',
        width: 100,
        render: (value:any) => {
    ...
    viewMap: {
      'main': ['name', 'type', 'vtepNum', 'shareType', 'createDate'],
      'select': ['name', 'type', 'vtepNum', 'createDate'],
    }
  }), [intl, getServerTime])
