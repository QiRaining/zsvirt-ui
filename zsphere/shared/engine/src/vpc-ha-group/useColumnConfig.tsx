...
type IKey = 'name' | 'uuid' | 'vrRefs' | 'owner' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'vpcRouterCount', defaultMessage: 'Total VPC vRouters' }),
        i18nKey: 'vpcRouterCount',
        key: 'vrRefs',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('vrRefs', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('vrRefs', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'vrRefs', 'owner', 'createDate'],
      'select': [],
      'sub': [],
    }
  }), [intl, getServerTime])
