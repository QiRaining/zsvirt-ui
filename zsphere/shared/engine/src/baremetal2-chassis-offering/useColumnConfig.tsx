...
type IKey = 'name' | 'uuid' | 'cpuModelName' | 'cpuNum' | 'memorySize' | 'architecture' | 'provisionType' | 'state' | 'shareType' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'cpuModelName', defaultMessage: 'CPU Model' }),
        i18nKey: 'cpuModelName',
        key: 'cpuModelName',
        width: 140,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('cpuModelName', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('cpuModelName', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'cpuModelName', 'cpuNum', 'memorySize', 'architecture', 'provisionType', 'state', 'shareType', 'createDate'],
      'main&#x2F;toolbar': [],
      'select': ['name', 'cpuModelName', 'cpuNum', 'memorySize', 'architecture', 'provisionType', 'state', 'createDate'],
      'sub': ['name', 'cpuModelName', 'cpuNum', 'memorySize', 'architecture', 'provisionType', 'state', 'shareType', 'createDate'],
      'sub.shared.resource': ['name', 'provisionType', 'shareType'],
    }
  }), [intl, getServerTime])
