...
type IKey = 'name' | 'type' | 'distribution' | 'state' | 'owner' | 'createDate' | 'uuid' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'distribution', defaultMessage: 'Operating System' }),
        i18nKey: 'distribution',
        key: 'distribution',
        width: 200,sorter: true,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('distribution', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('distribution', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'type', 'distribution', 'owner', 'createDate'],
      'select': ['name', 'type', 'distribution', 'owner', 'createDate'],
      'sub': ['name', 'type', 'distribution', 'owner', 'createDate'],
    }
  }), [intl, getServerTime])
