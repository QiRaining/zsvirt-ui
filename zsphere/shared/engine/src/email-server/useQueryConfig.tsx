...
type IKey = 'name' | 'smtpServer' | 'smtpPort'
...
const useQueryConfig = (
  ...
  const remoteConfig = useRemoteConfig('email-server')
  ...
  const originCandidates = useMemo(() => {
    const list: ICandidate[] = [
      ...
      {
        label: intl.formatMessage({ id: 'smtp.server', defaultMessage: 'SMTP Server' }),
        key: 'smtpServer',
        type: 'input'
      },
      {
        label: intl.formatMessage({ id: 'smtp.port', defaultMessage: 'SMTP Port' }),
        key: 'smtpPort',
        type: 'input'
      },
    ]
