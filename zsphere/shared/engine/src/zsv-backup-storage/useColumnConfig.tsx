...
type IKey = 'name' | 'state' | 'status' | 'image.server.capacity' | 'type' | 'hostname' | 'url' | 'createDate' | 'uuid' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'backup.server.capacity', defaultMessage: 'Backup Storage Capacity' }),
        i18nKey: 'backup.server.capacity',
        key: 'image.server.capacity',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('image.server.capacity', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('image.server.capacity', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
      ...
      {
        title: intl.formatMessage({ id: 'backup.node.ip', defaultMessage: 'Backup Storage IP' }),
        i18nKey: 'backup.node.ip',
        key: 'hostname',
        width: 140,
        render: (value:any) => {
          const renderText = (value: any) => {
    ...
    viewMap: {
      'main': ['name', 'state', 'status', 'image.server.capacity', 'type', 'hostname', 'url', 'createDate'],
      'select': ['name', 'state', 'status', 'image.server.capacity', 'type', 'hostname', 'url', 'createDate'],
      'sub': [],
    }
  }), [intl, getServerTime])
