...
type IKey = 'AccessKeyID' | 'uuid' | 'AccessKeySecret' | 'state' | 'owner' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      {
        title: intl.formatMessage({ id: 'accesskeyID', defaultMessage: 'AccessKey ID' }),
        i18nKey: 'accesskeyID',
        key: 'AccessKeyID',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('AccessKeyID', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('AccessKeyID', options)?.extra
          if (renderExtra) {
            return (
      ...
      {
        title: intl.formatMessage({ id: 'accesskey.secret', defaultMessage: 'AccessKey Secret' }),
        i18nKey: 'accesskey.secret',
        key: 'AccessKeySecret',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('AccessKeySecret', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('AccessKeySecret', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['AccessKeyID', 'AccessKeySecret', 'state', 'owner', 'createDate'],
      'select': [],
      'sub': [],
      'sub.vm': [],
    }
