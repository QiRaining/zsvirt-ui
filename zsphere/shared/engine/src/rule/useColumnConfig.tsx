...
type IKey = 'uuid' | 'ruleNumber' | 'isDefault' | 'protocol' | 'action' | 'state' | 'ruleSetName' | 'sourceIp' | 'destIp' | 'sourcePort' | 'destPort' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'rule.type', defaultMessage: 'Rule Type' }),
        i18nKey: 'rule.type',
        key: 'isDefault',
        width: 140,filters: [],
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('isDefault', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('isDefault', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
      ...
      {
        title: intl.formatMessage({ id: 'rule.set.name', defaultMessage: 'Associated Rule Set' }),
        i18nKey: 'rule.set.name',
        key: 'ruleSetName',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
            const linkResource = getOption('ruleSetName', options)?.linkResource || ''
            const val = formatValue('ruleSetName', value, options)
            return val || val === 0 ? (
              <Text>
                <Link to={ linkResource } uuid={formatLinkUuid('ruleSetName', value, options)} >{ val }</Link>
              </Text>
            ) : null
          }
          const renderExtra = getOption('ruleSetName', options)?.extra
          if (renderExtra) {
            return (
    ...
    viewMap: {
      'main': [],
      'select': [],
      'sub': ['ruleNumber', 'protocol', 'action', 'state', 'createDate'],
      'sub.firewall': ['ruleNumber', 'isDefault', 'protocol', 'action', 'state', 'ruleSetName'],
    }
  }), [intl, getServerTime])
