...
type IKey = 'name' | 'uuid' | 'rules' | 'owner' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'rules', defaultMessage: 'Rules' }),
        i18nKey: 'rules',
        key: 'rules',
        width: 100,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('rules', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('rules', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'rules', 'owner', 'createDate'],
      'select': ['name', 'rules', 'createDate'],
      'sub': ['name', 'rules', 'createDate'],
    }
  }), [intl, getServerTime])
