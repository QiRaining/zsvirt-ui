...
type IKey = 'uuid' | 'name' | 'descritpion' | 'state' | 'type' | 'model' | 'securityMachineNum' | 'createDate' | 'lastOpDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'descritpion', defaultMessage: 'Description' }),
        i18nKey: 'descritpion',
        key: 'descritpion',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('descritpion', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('descritpion', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
      ...
      {
        title: intl.formatMessage({ id: 'secretResourcePoolState', defaultMessage: 'State' }),
        i18nKey: 'secretResourcePoolState',
        key: 'state',
        width: 140,filters: [],
        render: (value: any) => {
          const renderText = (value: any) => {
      ...
      {
        title: intl.formatMessage({ id: 'secretResourcePoolType', defaultMessage: 'HSM Type' }),
        i18nKey: 'secretResourcePoolType',
        key: 'type',
        width: 140,
        render: (value:any) => {
          const renderText = (value: any) => {
      ...
      {
        title: intl.formatMessage({ id: 'secretResourcePoolModel', defaultMessage: 'HSM Model' }),
        i18nKey: 'secretResourcePoolModel',
        key: 'model',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
      ...
      {
        title: intl.formatMessage({ id: 'securityMachineNum', defaultMessage: 'HSMs' }),
        i18nKey: 'securityMachineNum',
        key: 'securityMachineNum',
        width: 100,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('securityMachineNum', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('securityMachineNum', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'state', 'type', 'model', 'securityMachineNum'],
      'select.platform.crypto.cmpl': ['name', 'state', 'type', 'model', 'securityMachineNum'],
    }
  }), [intl, getServerTime])
