...
type IKey = 'name' | 'uuid' | 'executionStrategy' | 'startTime' | 'state' | 'owner' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'executeStrategy', defaultMessage: 'Execution Policy' }),
        i18nKey: 'executeStrategy',
        key: 'executionStrategy',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('executionStrategy', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('executionStrategy', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'executionStrategy', 'startTime', 'state', 'owner', 'createDate'],
      'main.completed': ['name', 'executionStrategy', 'startTime', 'state', 'owner', 'createDate'],
      'select': ['name', 'executionStrategy', 'startTime', 'createDate'],
      'sub': ['name', 'executionStrategy', 'startTime', 'createDate'],
    }
  }), [intl, getServerTime])
