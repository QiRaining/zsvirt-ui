...
type IKey = 'vtepIp' | 'host' | 'type' | 'port' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      {
        title: intl.formatMessage({ id: 'vtep.ip', defaultMessage: 'VTEP IP' }),
        i18nKey: 'vtep.ip',
        key: 'vtepIp',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('vtepIp', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('vtepIp', options)?.extra
          if (renderExtra) {
            return (
    ...
    viewMap: {
      'main': ['vtepIp', 'host', 'type', 'port', 'createDate'],
      'select': [],
      'sub': [],
    }
  }), [intl, getServerTime])
