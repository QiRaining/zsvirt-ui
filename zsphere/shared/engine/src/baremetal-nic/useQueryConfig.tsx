...
const useQueryConfig = (
  ...
  const remoteConfig = useRemoteConfig('baremetal-nic')
  ...
  const originCandidates = useMemo(() => {
    const list: ICandidate[] = [
      {
        label: intl.formatMessage({ id: 'mac', defaultMessage: 'MAC' }),
        key: 'mac',
        type: 'input'
      },
      {
        label: intl.formatMessage({ id: 'baremetal-nic.uuid', defaultMessage: 'UUID' }),
        key: 'uuid',
        type: 'input'
      },
    ]
