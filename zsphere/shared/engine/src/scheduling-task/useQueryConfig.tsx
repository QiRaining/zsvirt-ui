...
const useQueryConfig = (
  ...
  const remoteConfig = useRemoteConfig('scheduling-task')
  ...
  const originCandidates = useMemo(() => {
    const list: ICandidate[] = [
      {
        label: intl.formatMessage({ id: 'migration.object', defaultMessage: 'Migration Object' }),
        key: 'job.object',
        type: 'input'
      },
      {
        label: intl.formatMessage({ id: 'virtualization.UUID', defaultMessage: 'UUID' }),
        key: 'uuid',
        type: 'input'
      },
    ]
