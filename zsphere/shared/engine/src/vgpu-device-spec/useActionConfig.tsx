...
type IKey = 'enable' | 'disable' | 'set.share.type' | 'cancel.share'
...
function useActionConfig<T extends Item, K extends Item = Item>(
  ...
  const _actionConfig: Required<ITableListProps<T, K>>['actionConfig'] = useMemo(() => ({
    ...
    viewMap: {
      ...
      'main/row': {
        extraKeys: [],
        activeKeys: ['enable', 'disable', 'set.share.type'],
      },
      'main/toolbar': {
        extraKeys: ['enable', 'disable'],
        activeKeys: ['set.share.type'],
  ...
  useEffect(() => {
    if (localStorage.getItem('debug-branch')) {
      genActionFromRemote('vgpu-device-spec', intl).then(remoteConfig => {
