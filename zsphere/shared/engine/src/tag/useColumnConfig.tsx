...
type IKey = 'name' | 'uuid' | 'color' | 'count' | 'owner' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'color', defaultMessage: 'Color' }),
        i18nKey: 'color',
        key: 'color',
        width: 100,sorter: true,filters: [],
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('color', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('color', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
      ...
      {
        title: intl.formatMessage({ id: 'bind.resource.count', defaultMessage: 'Attached Resources' }),
        i18nKey: 'bind.resource.count',
        key: 'count',
        width: 140,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('count', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('count', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'count', 'owner', 'createDate'],
      'main.virtualization': ['name', 'count', 'owner', 'createDate'],
      'select': ['name', 'count', 'owner', 'createDate'],
      'sub': [],
      'sub.create': ['name', 'count', 'owner', 'createDate'],
      'sub.vm': [],
    }
  }), [intl, getServerTime])
