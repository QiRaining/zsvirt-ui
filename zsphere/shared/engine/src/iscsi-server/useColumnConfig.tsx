...
type IKey = 'name' | 'uuid' | 'iqn.count' | 'ip' | 'port' | 'state' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'iqn.count', defaultMessage: 'IQNs' }),
        i18nKey: 'iqn.count',
        key: 'iqn.count',
        width: 100,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('iqn.count', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('iqn.count', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'iqn.count', 'ip', 'port', 'state', 'createDate'],
      'select': [],
      'select.cluster': ['name', 'ip', 'port', 'state', 'createDate'],
      'sub': [],
      'sub.baremetal2Cluster': ['name', 'ip', 'port', 'state', 'createDate'],
      'sub.cluster': ['name', 'ip', 'port', 'state', 'createDate'],
    }
  }), [intl, getServerTime])
