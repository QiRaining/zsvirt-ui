...
type IKey = 'poolName' | 'primaryStorage' | 'aliasName' | 'virtualCapacityAllocationRatio' | 'capacityUtilization' | 'usedCapacity' | 'availableCapacity' | 'securityPolicy' | 'type' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      {
        ...
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('poolName', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('poolName', options)?.extra
          if (renderExtra) {
            return (
      ...
      {
        title: intl.formatMessage({ id: 'displayName', defaultMessage: 'Display Name' }),
        i18nKey: 'displayName',
        key: 'aliasName',
        width: 200,
        render: (value:any) => {
      ...
      },
      {
        ...
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('capacityUtilization', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('capacityUtilization', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
      ...
      {
        title: intl.formatMessage({ id: 'usedCapacity', defaultMessage: 'Capacity Used' }),
        i18nKey: 'usedCapacity',
        key: 'usedCapacity',
        width: 140,
        render: (value:any) => {
          const renderText = (value: any) => {
    ...
    viewMap: {
      'main': [],
      'select': ['poolName', 'aliasName', 'virtualCapacityAllocationRatio', 'createDate'],
      'select.alarm': ['poolName', 'primaryStorage', 'virtualCapacityAllocationRatio', 'capacityUtilization', 'securityPolicy', 'type', 'createDate'],
      'sub': ['poolName', 'aliasName', 'virtualCapacityAllocationRatio', 'capacityUtilization', 'usedCapacity', 'availableCapacity', 'securityPolicy', 'type', 'createDate'],
      'sub.alarm': ['poolName', 'primaryStorage', 'virtualCapacityAllocationRatio', 'capacityUtilization', 'securityPolicy', 'type', 'createDate'],
      'sub.primary.storage': ['poolName', 'aliasName', 'virtualCapacityAllocationRatio', 'capacityUtilization', 'securityPolicy', 'type', 'createDate'],
      'sub.virtualization.primary.storage': ['poolName', 'aliasName', 'virtualCapacityAllocationRatio', 'capacityUtilization', 'securityPolicy', 'type', 'createDate'],
    }
  }), [intl, getServerTime])
