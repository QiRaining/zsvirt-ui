...
type IKey = 'name' | 'uuid' | 'version' | 'expireInterval' | 'vRouterCount' | 'ip' | 'port' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'expireInterval', defaultMessage: 'Export Interval' }),
        i18nKey: 'expireInterval',
        key: 'expireInterval',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('expireInterval', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('expireInterval', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
      ...
      {
        title: intl.formatMessage({ id: 'vRouterCount', defaultMessage: 'VPC vRouters' }),
        i18nKey: 'vRouterCount',
        key: 'vRouterCount',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('vRouterCount', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('vRouterCount', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'version', 'expireInterval', 'vRouterCount', 'ip', 'port', 'createDate'],
      'select': [],
      'sub': [],
    }
  }), [intl, getServerTime])
