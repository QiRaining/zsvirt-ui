...
type IKey = 'name' | 'uuid' | 'applicationPlatformType' | 'type' | 'createDate' | '__action__'
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      },
      {
        ...
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('applicationPlatformType', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('applicationPlatformType', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
      ...
      {
        title: intl.formatMessage({ id: 'alarm.type', defaultMessage: 'Alarm Type' }),
        i18nKey: 'alarm.type',
        key: 'type',
        width: 140,
        render: (value:any) => {
    ...
    viewMap: {
      'main': ['name', 'applicationPlatformType', 'type', 'createDate'],
      'main.virtualization': ['name', 'applicationPlatformType', 'type', 'createDate'],
      'select': ['name', 'applicationPlatformType', 'type', 'createDate'],
      'sub': ['name', 'applicationPlatformType', 'type', 'createDate'],
      'virtualization.main': ['name', 'applicationPlatformType', 'type', 'createDate'],
    }
  }), [intl, getServerTime])
