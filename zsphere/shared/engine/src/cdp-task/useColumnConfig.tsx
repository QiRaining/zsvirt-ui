...
type IKey = 'name' | 'uuid' | 'state' | 'status' | 'vmInstance' | 'cdpCapacityUtilization' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'cdpTaskStatus', defaultMessage: 'Task Status' }),
        i18nKey: 'cdpTaskStatus',
        key: 'status',
        width: 140,filters: [],
        render: (value: any) => {
          const renderText = (value: any) => {
      ...
      {
        title: intl.formatMessage({ id: 'cdpCapacityUtilization', defaultMessage: 'CDP Capacity Utilization' }),
        i18nKey: 'cdpCapacityUtilization',
        key: 'cdpCapacityUtilization',
        width: 200,sorter: true,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('cdpCapacityUtilization', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('cdpCapacityUtilization', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'state', 'status', 'vmInstance', 'cdpCapacityUtilization', 'createDate'],
      'select': ['name', 'state', 'status', 'vmInstance', 'cdpCapacityUtilization', 'createDate'],
    }
  }), [intl, getServerTime])
