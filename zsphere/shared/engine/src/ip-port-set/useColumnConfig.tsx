...
type IKey = 'name' | 'uuid' | 'type' | 'sourceValue' | 'destValue' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'sourceValue', defaultMessage: 'Source IP Address/Port' }),
        i18nKey: 'sourceValue',
        key: 'sourceValue',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('sourceValue', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('sourceValue', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
      ...
      {
        title: intl.formatMessage({ id: 'destValue', defaultMessage: 'Destination IP/Port' }),
        i18nKey: 'destValue',
        key: 'destValue',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('destValue', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('destValue', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'type', 'sourceValue', 'destValue', 'createDate'],
      'select.rule.create': ['name', 'sourceValue', 'destValue'],
      'sub': [],
    }
  }), [intl, getServerTime])
