...
const useQueryConfig = (
  ...
  const remoteConfig = useRemoteConfig('route-entry')
  ...
  const originCandidates = useMemo(() => {
    const list: ICandidate[] = [
      {
        label: intl.formatMessage({ id: 'destination', defaultMessage: 'Destination Network' }),
        key: 'destination',
        type: 'input'
      },
      {
        label: intl.formatMessage({ id: 'next.hop', defaultMessage: 'Next Hop' }),
        key: 'target',
        type: 'input'
      },
    ]
