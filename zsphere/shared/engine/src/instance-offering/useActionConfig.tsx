...
type IKey = 'create' | 'edit' | 'start' | 'stop' | 'set.share.type' | 'delete' | 'cancel.share'
...
function useActionConfig<T extends Item, K extends Item = Item>(
  ...
  const _actionConfig: Required<ITableListProps<T, K>>['actionConfig'] = useMemo(() => ({
    list: [
      {
        key: 'create',
        name: intl.formatMessage({ id: 'create.instanceOffering', defaultMessage: 'Create Instance Offering' }),
        auth: {
          authKey: 'create',
          resource: 'instance.offering',
          type: 'action'
        },
        icon: 'plus',
      },
  ...
  useEffect(() => {
    if (localStorage.getItem('debug-branch')) {
      genActionFromRemote('instance-offering', intl).then(remoteConfig => {
