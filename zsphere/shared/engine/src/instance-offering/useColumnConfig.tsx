...
type IKey = 'name' | 'uuid' | 'cpuNum' | 'memorySize' | 'state' | 'allocatorStrategy' | 'shareType' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'host.allocator.strategy', defaultMessage: 'Host Allocation Strategy' }),
        i18nKey: 'host.allocator.strategy',
        key: 'allocatorStrategy',
        width: 200,filters: [],
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('allocatorStrategy', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('allocatorStrategy', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'cpuNum', 'memorySize', 'state', 'allocatorStrategy', 'shareType', 'createDate'],
      'select': ['name', 'cpuNum', 'memorySize', 'state', 'allocatorStrategy', 'shareType', 'createDate'],
      'select.auto.scaling': ['name', 'cpuNum', 'memorySize', 'allocatorStrategy', 'createDate'],
      'sub': ['name', 'cpuNum', 'memorySize', 'state', 'allocatorStrategy', 'shareType', 'createDate'],
      'sub.shared.resource': ['name', 'shareType'],
      'sub.vm': ['name', 'cpuNum', 'memorySize', 'state', 'allocatorStrategy', 'shareType', 'createDate'],
    }
  }), [intl, getServerTime])
