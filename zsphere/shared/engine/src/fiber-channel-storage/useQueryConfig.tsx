...
type IKey = 'name' | 'uuid' | 'fiberChannelLun.uuid'
...
const useQueryConfig = (
  ...
  const remoteConfig = useRemoteConfig('fiber-channel-storage')
  ...
  const originCandidates = useMemo(() => {
    const list: ICandidate[] = [
      ...
      {
        label: intl.formatMessage({ id: 'fc.lun.uuid', defaultMessage: 'LUN UUID' }),
        key: 'fiberChannelLun.uuid',
        type: 'input'
      },
    ]
    if (filteredKeys) {
