...
type IKey = 'eventName' | 'uuid' | 'namespace' | 'emergencyLevel' | 'state' | 'topicNum' | 'owner' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      {
        ...
        render: (value:any) => {
          const renderText = (value: any) => {
            const linkResource = getOption('eventName', options)?.linkResource || ''
            const val = formatValue('eventName', value, options)
            return val || val === 0 ? (
              <Text>
                <Link to={ linkResource } uuid={formatLinkUuid('eventName', value, options)} >{ val }</Link>
              </Text>
            ) : null
          }
          const renderExtra = getOption('eventName', options)?.extra
          if (renderExtra) {
            return (
    ...
    viewMap: {
      'main': ['eventName', 'namespace', 'emergencyLevel', 'state', 'topicNum', 'owner', 'createDate'],
      'main.virtualization': ['eventName', 'namespace', 'emergencyLevel', 'state', 'topicNum', 'owner', 'createDate'],
      'select': ['eventName', 'namespace', 'emergencyLevel', 'state', 'topicNum', 'owner', 'createDate'],
      'sub': ['eventName', 'namespace', 'emergencyLevel', 'state', 'topicNum', 'owner', 'createDate'],
      'sub.virtualization': ['eventName', 'namespace', 'emergencyLevel', 'state', 'topicNum', 'owner', 'createDate'],
      'virtualization.main': ['eventName', 'namespace', 'emergencyLevel', 'state', 'topicNum', 'owner', 'createDate'],
    }
  }), [intl, getServerTime])
