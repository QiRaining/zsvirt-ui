...
type IKey = 'areaId' | 'uuid' | 'type' | 'authentication' | 'owner' | 'shareType' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      {
        title: intl.formatMessage({ id: 'areaId', defaultMessage: 'Area ID' }),
        i18nKey: 'areaId',
        key: 'areaId',
        width: 200,sorter: true,
        render: (value:any) => {
          const renderText = (value: any) => {
            const linkResource = getOption('areaId', options)?.linkResource || ''
            const val = formatValue('areaId', value, options)
            return val || val === 0 ? (
              <Text>
                <Link to={ linkResource } uuid={formatLinkUuid('areaId', value, options)} >{ val }</Link>
              </Text>
            ) : null
          }
          const renderExtra = getOption('areaId', options)?.extra
          if (renderExtra) {
            return (
      ...
      {
        title: intl.formatMessage({ id: 'authType', defaultMessage: 'Authentication' }),
        i18nKey: 'authType',
        key: 'authentication',
        width: 140,filters: [],
        render: (value: any) => {
          const renderText = (value: any) => {
            const val = formatValue('authentication', value, options)
            return val || val === 0 ? <Constant value={ val } /> : null
          }
          const renderExtra = getOption('authentication', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['areaId', 'type', 'authentication', 'owner', 'shareType', 'createDate'],
      'select': ['areaId', 'type', 'authentication', 'owner', 'createDate'],
      'sub': ['areaId', 'type', 'authentication', 'owner', 'createDate'],
    }
  }), [intl, getServerTime])
