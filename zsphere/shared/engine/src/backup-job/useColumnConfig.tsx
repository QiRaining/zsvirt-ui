...
type IKey = 'name' | 'state' | 'lastJobResult' | 'jobType' | 'resourceNum' | 'status' | 'owner' | 'createDate' | 'uuid' | '__action__'
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      },
      {
        ...
        render: (value: any) => {
          const renderText = (value: any) => {
            const val = formatValue('lastJobResult', value, options)
            return val || val === 0 ? <Constant value={ val } /> : null
          }
          const renderExtra = getOption('lastJobResult', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
      ...
      },
      {
        ...
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('resourceNum', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('resourceNum', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'state', 'lastJobResult', 'jobType', 'resourceNum', 'owner', 'createDate'],
      'select': ['name', 'state', 'jobType', 'resourceNum', 'owner', 'createDate'],
      'sub': ['name', 'state', 'lastJobResult', 'jobType', 'resourceNum', 'createDate'],
    }
  }), [intl, getServerTime])
