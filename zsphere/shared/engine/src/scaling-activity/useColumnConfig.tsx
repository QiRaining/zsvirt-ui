...
type IKey = 'activityAction' | 'uuid' | 'status' | 'createDate' | 'endDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      {
        title: intl.formatMessage({ id: 'activity.action', defaultMessage: 'Actions' }),
        i18nKey: 'activity.action',
        key: 'activityAction',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('activityAction', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('activityAction', options)?.extra
          if (renderExtra) {
            return (
      ...
      {
        title: intl.formatMessage({ id: 'endDate', defaultMessage: 'End Time' }),
        i18nKey: 'endDate',
        key: 'endDate',
        width: 200,sorter: true,
        render: (value: any) => {
          const renderText = (value: any) => {
            const val = formatValue('endDate', value, options)
            return val || val === 0 ? <Text>{getServerTime(val).format('YYYY-MM-DD HH:mm:ss')}</Text> : null
          }
          const renderExtra = getOption('endDate', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': [],
      'select': [],
      'sub': ['activityAction', 'status', 'createDate', 'endDate'],
    }
  }), [intl, getServerTime])
