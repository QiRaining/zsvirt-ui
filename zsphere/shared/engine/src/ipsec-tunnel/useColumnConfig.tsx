...
type IKey = 'name' | 'uuid' | 'vipRef.ip' | 'peerAddress' | 'state' | 'status' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'local.public.network.ip', defaultMessage: 'Local Public IP' }),
        i18nKey: 'local.public.network.ip',
        key: 'vipRef.ip',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('vipRef.ip', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('vipRef.ip', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
      ...
      {
        title: intl.formatMessage({ id: 'remote.public.network.ip', defaultMessage: 'Remote Public IP' }),
        i18nKey: 'remote.public.network.ip',
        key: 'peerAddress',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('peerAddress', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('peerAddress', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'vipRef.ip', 'peerAddress', 'state', 'status', 'createDate'],
      'select': ['name', 'vipRef.ip', 'peerAddress', 'status', 'createDate'],
      'sub': ['name', 'vipRef.ip', 'peerAddress', 'state', 'status', 'createDate'],
      'sub.vpc': ['name', 'vipRef.ip', 'peerAddress', 'state', 'status', 'createDate'],
    }
  }), [intl, getServerTime])
