...
type IKey = 'name' | 'uuid' | 'vipRef.ip' | 'peerAddress'
...
const useQueryConfig = (
  ...
  const remoteConfig = useRemoteConfig('ipsec-tunnel')
  ...
  const originCandidates = useMemo(() => {
    const list: ICandidate[] = [
      ...
      {
        label: intl.formatMessage({ id: 'local.public.network.ip', defaultMessage: 'Local Public IP' }),
        key: 'vipRef.ip',
        type: 'input'
      },
      {
        label: intl.formatMessage({ id: 'remote.public.network.ip', defaultMessage: 'Remote Public IP' }),
        key: 'peerAddress',
        type: 'input'
      },
    ]
