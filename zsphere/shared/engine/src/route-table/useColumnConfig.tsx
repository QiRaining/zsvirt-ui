...
type IKey = 'name' | 'uuid' | 'routeEntries' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'route.entries.count', defaultMessage: 'Route Entries' }),
        i18nKey: 'route.entries.count',
        key: 'routeEntries',
        width: 140,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('routeEntries', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('routeEntries', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'routeEntries', 'createDate'],
      'select': [],
      'sub': [],
    }
  }), [intl, getServerTime])
