...
type IKey = 'name' | 'uuid' | 'vipIp' | 'guestIp' | 'vmNic.vmInstance.name' | 'owner'
...
const useQueryConfig = (
  ...
  const remoteConfig = useRemoteConfig('eip')
  ...
  const originCandidates = useMemo(() => {
    const list: ICandidate[] = [
      ...
      {
        label: intl.formatMessage({ id: 'vip.ip', defaultMessage: 'Public IP' }),
        key: 'vipIp',
        type: 'input'
      },
      {
        label: intl.formatMessage({ id: 'guestIp', defaultMessage: 'Private IP' }),
        key: 'guestIp',
        type: 'input'
      },
      {
        label: intl.formatMessage({ id: 'instance', defaultMessage: 'Virtual Machine' }),
