...
type IKey = 'name' | 'uuid' | 'vipIp' | 'guestIp' | 'vmNic.vmInstance.name' | 'owner' | 'state' | 'createDate' | 'lastOpDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'vip.ip', defaultMessage: 'Public IP' }),
        i18nKey: 'vip.ip',
        key: 'vipIp',
        width: 200,
        render: (value:any) => {
      ...
      {
        title: intl.formatMessage({ id: 'guestIp', defaultMessage: 'Private IP' }),
        i18nKey: 'guestIp',
        key: 'guestIp',
        width: 200,
        render: (value:any) => {
      ...
      {
        title: intl.formatMessage({ id: 'instance', defaultMessage: 'Virtual Machine' }),
        i18nKey: 'instance',
        key: 'vmNic.vmInstance.name',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
            const linkResource = getOption('vmNic.vmInstance.name', options)?.linkResource || ''
            const val = formatValue('vmNic.vmInstance.name', value, options)
            return val || val === 0 ? (
              <Text>
                <Link to={ linkResource } uuid={formatLinkUuid('vmNic.vmInstance.name', value, options)} >{ val }</Link>
              </Text>
            ) : null
          }
          const renderExtra = getOption('vmNic.vmInstance.name', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap items-center gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'vipIp', 'guestIp', 'vmNic.vmInstance.name', 'owner', 'state', 'createDate'],
      'select': ['name', 'vipIp', 'owner', 'state', 'createDate'],
      'sub': ['name', 'vipIp', 'guestIp', 'vmNic.vmInstance.name', 'state', 'createDate'],
      'sub.vpc': ['name', 'vipIp', 'guestIp', 'vmNic.vmInstance.name', 'state', 'createDate'],
    }
  }), [intl, getServerTime])
