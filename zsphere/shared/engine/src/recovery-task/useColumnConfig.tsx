...
type IKey = 'name' | 'uuid' | 'state' | 'vm' | 'executeTime' | 'recoveryPointTime' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'targetVM', defaultMessage: 'Target VM' }),
        i18nKey: 'targetVM',
        key: 'vm',
        width: 100,
        render: (value:any) => {
      ...
      {
        title: intl.formatMessage({ id: 'timeConsuming', defaultMessage: 'Time Consumed' }),
        i18nKey: 'timeConsuming',
        key: 'executeTime',
      ...
      {
        title: intl.formatMessage({ id: 'recoveryPointTime', defaultMessage: 'Recovery Point Time' }),
        i18nKey: 'recoveryPointTime',
        key: 'recoveryPointTime',
        width: 100,
        render: (value: any) => {
          const renderText = (value: any) => {
            const val = formatValue('recoveryPointTime', value, options)
            return val || val === 0 ? <Text>{getServerTime(val).format('YYYY-MM-DD HH:mm:ss')}</Text> : null
          }
          const renderExtra = getOption('recoveryPointTime', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'state', 'vm', 'executeTime', 'recoveryPointTime', 'createDate'],
      'sub': [],
    }
  }), [intl, getServerTime])
