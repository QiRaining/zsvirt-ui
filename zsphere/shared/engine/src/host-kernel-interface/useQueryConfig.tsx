...
const useQueryConfig = (
  ...
  const remoteConfig = useRemoteConfig('host-kernel-interface')
  ...
  const originCandidates = useMemo(() => {
    const list: ICandidate[] = [
      {
        label: intl.formatMessage({ id: 'virtualization.zskernel.name', defaultMessage: 'Name' }),
        key: 'name',
        type: 'input'
      },
      {
        label: intl.formatMessage({ id: 'virtualization.zskernel.ipAddress', defaultMessage: 'IP Address' }),
        key: 'ipAddress',
        type: 'input'
      },
    ]
