...
type IKey = 'name' | 'description' | 'vendorId' | 'romVersion' | 'state' | 'shareType' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'romVersion', defaultMessage: 'ROM Version' }),
        i18nKey: 'romVersion',
        key: 'romVersion',
        width: 200,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('romVersion', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('romVersion', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'description', 'vendorId', 'romVersion', 'state', 'shareType', 'createDate'],
      'select': ['name', 'description', 'vendorId', 'romVersion', 'state', 'shareType', 'createDate'],
      'sub': ['name', 'description', 'vendorId', 'romVersion', 'state', 'shareType', 'createDate'],
