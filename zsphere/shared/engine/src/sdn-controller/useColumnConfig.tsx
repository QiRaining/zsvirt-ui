...
type IKey = 'name' | 'uuid' | 'ip' | 'vendorType' | 'createDate' | '__action__'
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      },
      {
        ...
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('vendorType', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('vendorType', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'ip', 'vendorType', 'createDate'],
      'select': ['name', 'ip', 'vendorType', 'createDate'],
      'sub': [],
    }
  }), [intl, getServerTime])
