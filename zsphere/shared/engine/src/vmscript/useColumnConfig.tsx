...
type IKey = 'name' | 'platform' | 'scriptType' | 'createDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'platformType', defaultMessage: 'Platform Type' }),
        i18nKey: 'platformType',
        key: 'platform',
        width: 100,filters: [],
        render: (value: any) => {
          const renderText = (value: any) => {
      ...
      {
        title: intl.formatMessage({ id: 'scriptType', defaultMessage: 'Script Type' }),
        i18nKey: 'scriptType',
        key: 'scriptType',
        width: 100,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('scriptType', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('scriptType', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': ['name', 'platform', 'scriptType', 'createDate'],
    }
  }), [intl, getServerTime])
