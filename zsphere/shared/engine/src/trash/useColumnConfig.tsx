...
type IKey = 'uuid' | 'size' | 'trashType' | 'installPath' | 'createDate' | '__action__'
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      },
      {
        ...
        render: (value: any) => {
          const renderText = (value: any) => {
            const val = formatValue('trashType', value, options)
            return val || val === 0 ? <Constant value={ val } /> : null
          }
          const renderExtra = getOption('trashType', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
    ...
    viewMap: {
      'main': [],
      'select': ['uuid', 'size', 'trashType', 'installPath', 'createDate'],
      'sub': ['uuid', 'size', 'trashType', 'installPath', 'createDate'],
      'sub.backup.storage': ['uuid', 'size', 'trashType', 'installPath', 'createDate'],
      'sub.primary.storage': ['uuid', 'size', 'trashType', 'installPath', 'createDate'],
    }
  }), [intl, getServerTime])
