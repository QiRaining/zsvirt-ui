...
const useQueryConfig = (
  ...
  const remoteConfig = useRemoteConfig('vcenter-resource-pool')
  ...
  const originCandidates = useMemo(() => {
    const list: ICandidate[] = [
      {
        label: intl.formatMessage({ id: 'vcenter-resource-pool.name', defaultMessage: 'Name' }),
        key: 'name',
        type: 'input'
      },
      {
        label: intl.formatMessage({ id: 'vcenter-resource-pool.uuid', defaultMessage: 'uuid' }),
        key: 'uuid',
        type: 'input'
      },
    ]
