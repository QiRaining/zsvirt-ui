...
type IKey = 'uuid' | 'name' | 'description' | 'state' | 'secretResourcePool' | 'status' | 'managementIp' | 'port' | 'createDate' | 'lastOpDate' | '__action__'
...
function useColumnConfig<T extends Item>(
  ...
  const _columnConfig: ITableListProps<T>['columnConfig'] = useMemo(() => ({
    list: [
      ...
      {
        title: intl.formatMessage({ id: 'secretResourcePool', defaultMessage: 'HSM Pool' }),
        i18nKey: 'secretResourcePool',
        key: 'secretResourcePool',
        width: 100,
        render: (value:any) => {
          const renderText = (value: any) => {
            const val = formatValue('secretResourcePool', value, options)
            return val || val === 0 ? <Text>{ val }</Text> : null
          }
          const renderExtra = getOption('secretResourcePool', options)?.extra
          if (renderExtra) {
            return (
              <div className="flex flex-nowrap gap-2">
                <div className="min-w-0 flex-auto">
      ...
      {
        title: intl.formatMessage({ id: 'managementIp', defaultMessage: 'IP' }),
    ...
    viewMap: {
      'main': ['name', 'state', 'secretResourcePool', 'status', 'managementIp', 'port', 'createDate'],
      'sub.secretResourcePool': ['name', 'state', 'secretResourcePool', 'status', 'createDate'],
    }
  }), [intl, getServerTime])
