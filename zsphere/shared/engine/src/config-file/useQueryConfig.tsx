...
const useQueryConfig = (
  ...
  const remoteConfig = useRemoteConfig('config-file')
  ...
  const originCandidates = useMemo(() => {
    const list: ICandidate[] = [
      {
        label: intl.formatMessage({ id: 'vm.config.name', defaultMessage: 'Name' }),
        key: 'name',
        type: 'input'
      },
      {
        label: intl.formatMessage({ id: 'metadata.path', defaultMessage: 'Path' }),
        key: 'path',
        type: 'input'
      },
    ]
