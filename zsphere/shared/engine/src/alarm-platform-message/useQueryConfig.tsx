...
type IKey = 'dataUuid' | 'resourceUuid' | 'resourceName'
...
const useQueryConfig = (
  ...
  const remoteConfig = useRemoteConfig('alarm-platform-message')
  ...
  const originCandidates = useMemo(() => {
    const list: ICandidate[] = [
      {
        label: intl.formatMessage({ id: 'messageUuid', defaultMessage: 'Message UUID' }),
        key: 'dataUuid',
        type: 'input'
      },
      {
      ...
      {
        label: intl.formatMessage({ id: 'alarm.message.resource', defaultMessage: 'Resource' }),
        key: 'resourceName',
        type: 'input'
      },
    ]
