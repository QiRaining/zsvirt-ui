...
function fixUseColumnConfig(filePath) {
  ...
  if (
    content.includes(
      "function useColumnConfig<T extends { [prop: string]: any }>",
    )
  ) {
    content = content.replace(
...
function fixUseQueryConfig(filePath) {
  ...
  if (
    content.includes("mergeCandidates(") &&
    !content.includes("originCandidates as any")
  ) {
    content = content.replace(
  ...
  // 修复 setCandidates 调用
  if (
    content.includes("fuzzyConfig.setCandidates(queryConfig)") &&
    !content.includes("queryConfig as any")
  ) {
    content = content.replace(
