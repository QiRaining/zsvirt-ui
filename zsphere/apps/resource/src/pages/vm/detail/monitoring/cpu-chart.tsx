...
const CPUChart: FC<IProps> = ({
  ...
  const { realLabels, metricNames, metricNamesWithoutLabel } = useMemo(() => {
    ...
    if (zwatchState !== "Running") {
      _metricNames = _metricNames.filter(
        (item) => !item.startsWith("OperatingSystem"),
      );
      _metricNamesWithoutLabel = _metricNamesWithoutLabel.filter(
        (item) => !item.startsWith("OperatingSystem"),
      );
    }
    return {
  ...
  return (
    <MonitorChart
      ...
      isEmpty={isEmpty || !metricNames.concat(metricNamesWithoutLabel).length}
      {...props}
    />
  );
};
