      ...
      {() => {
        ...
        const infoShow = (hardwareType: IHardwareType) => {
          ...
          if (hardwareType === IHardwareType.CPU) {
            ...
            if (cpuNum) {
              info = `${form?.getFieldValue(updateFieldName)} ${intl.formatMessage(
                {
                  id: "core",
          ...
          if ([IHardwareType.Netcard].indexOf(hardwareType) !== -1) {
            //
            const nic =
              origin && currentUser?.currentIdentity !== "Admin"
                ? intl.formatMessage(
                    {
                ...
                : form?.getFieldValue(updateFieldName)?.[0]?.name;
        ...
        const showError = (hardwareType: IHardwareType) => {
          ...
          if (hardwareType === IHardwareType.Netcard) {
            const nic = origin || form?.getFieldValue(updateFieldName)?.[0];
            if (!nic) {
              return true;
            }
                  ...
                  canDelete && (
                    <Icon
                      ...
                      onClick={() => {
                        ...
                        // 触发硬件信息Tab Header的校验
                        form.setFieldsValue({ hardwareTitleUpdate: genUuid() });
                      }}
                    />
                  )}
              </div>
