...
const STYLE_MARGIN_TOP_18 = { marginTop: 18 } as const;
...
const _allCanDisableItems = ["gpuDeviceType", "gpuDevice"];
...
const GPUCard: React.FC<IProps> = ({
  ...
  const getDefaultQuery = (values: any) => {
    ...
    const selectedGpuUuids = keys(values)
      .filter(
        (key) =>
  ...
  return (
    <div className={styles.content}>
      ...
      <Item
        style={STYLE_MARGIN_TOP_18}
        shouldUpdate={(pre, cur) => {
          const keys = keys(cur).filter((key) => key.indexOf("gpuDevice") > -1);
          return keys.some((key) => pre[key] !== cur[key]);
        }}
      >
        {({ getFieldsValue }) => {
