...
interface IProps {
  ...
  cpuNum: number;
  isEdit: boolean;
  setCpuBindList: (cpuBindList: any[]) => void;
  cpuBindList: any[];
}

const { apolloClient } = window.g_main;

const genHostUuid = (source: any, hostUuid: string) => {
  if (hostUuid) {
    return hostUuid;
  }
  ...
  if (source?.__typename === "VmInstance") {
    return source?.host?.uuid || source?.lastHost?.uuid;
  }
  return "";
};
...
const BindCPUModal: React.FC<IProps> = ({
  ...
  const intl = useIntl();
  const uuid = genHostUuid(source, hostUuid);
  ...
  useEffect(() => {
    ...
    const haveCpuBindListByVCpu = !!(
      cpuBindListByVCpu?.every((it: any) => it.pCPUList.length !== 0) &&
      cpuBindListByVCpu?.length
    );

    if (visible) {
      if (haveCpuBindListByVCpu) {
        form.setFields([
          { name: "cpuBindListByVCpu", value: cpuBindListByVCpu },
          { name: "vnumaEnabled", value: true },
        ]);
      } else {
  ...
  const onCancel = () => {
    const haveCpuBindListByVCpu = form
      .getFieldValue("cpuBindListByVCpu")
      ?.every((it: any) => it.pCPUList.length !== 0);

    if (!haveCpuBindListByVCpu) {
      form.setFields([{ name: "cpuBindListByVCpu", value: [] }]);
    }

    if (!isEdit && !cpuBindList.length) {
      form.setFields([{ name: "cpuBindListByVCpu", value: [] }]);
  ...
  const onOk = () => {
    ...
    form
      .validateFields?.()
      .then(() => {
        setCpuBindList(form.getFieldValue("cpuBindListByVCpu"));
        setVisible(false);
      })
      .catch(({ errorFields = [] }) => {
