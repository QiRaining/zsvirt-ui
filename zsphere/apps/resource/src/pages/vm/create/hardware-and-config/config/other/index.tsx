...
const OtherConfig: React.FC<IProps> = ({ form }) => {
  ...
  // 监听runPath变化，更新hostUuid
  useEffect(() => {
    const runPath = form.getFieldValue("runPath");
    if (runPath?.[0]?.uuid && runPath?.[0]?.uuid !== hostUuid) {
      setHostUuid(runPath?.[0]?.uuid);
    } else if (!runPath?.[0]?.uuid && hostUuid) {
      // 如果runPath中没有uuid，清空hostUuid
      setHostUuid("");
    }
  }, [form, hostUuid]);

  // 监听runPath变化，设置haStickStragedy字段
  useEffect(() => {
    const runPath = form.getFieldValue("runPath");
  ...
  useEffect(() => {
    ...
    if (runPath?.[0] && runPath?.[0]?.__typename) {
      ...
      if (runPath?.[0]?.__typename === "HostVO") {
        emulateHyperV =
          runPath?.[0]?.cluster?.resourceConfigValue?.vmEmulateHyperV ===
          "true";
      }
      if (runPath?.[0]?.__typename === "Cluster") {
        emulateHyperV =
...
Default: enabled. Specifies whether to enable hot plugging of PCI devices for a VM. If a hardware incompatibility error occurs during hot plugging or a hardware device does not support hot plugging, you can turn off this switch.`,
      ...
      <Item
        noStyle
        shouldUpdate={(prev, curr) => prev.runPath !== curr.runPath}
      >
        {({ getFieldValue }) => {
          const runPath = getFieldValue("runPath")?.[0];
          if (
            !(
              runPath?.__typename === "HostVO" &&
              runPath.hostSystemInfo?.hostCpuModelName
                ?.toLowerCase()
                .includes("hygon")
            )
          ) {
          ...
          return (
            <Item
              ...
              iconTooltip={
                <ReactMarkdown>
                  {intl.formatMessage({
                    id: "vm.create.field.cpuid.vendor.tooltip",
                    defaultMessage:
