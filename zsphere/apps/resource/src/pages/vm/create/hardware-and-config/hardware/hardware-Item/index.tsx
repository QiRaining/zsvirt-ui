...
const HardwareItem: React.FC<IHardWareItem> = ({
  ...
  const titleIndex = String(displayIndex ?? index);
      ...
      {() => {
        ...
        const infoShow = (hardwareType: IHardwareType) => {
          ...
          if ([IHardwareType.Disk].indexOf(hardwareType) !== -1) {
            ...
            if (createType === "image") {
              ...
              const diskSize = form?.getFieldValue(`diskSize-${DiskIndex}`);
              // 优先显示 name，如果 name 不存在则显示容量大小
              if (diskImage) {
                info = diskImage;
              } else if (diskSize?.number) {
                info = formatBytesToSize(diskSize?.number, diskSize?.unit);
              } else {
                info = intl.formatMessage({
                  id: "disk.alert.in.hardware.item",
                  defaultMessage: "Not Configured",
          ...
          if ([IHardwareType.Netcard].indexOf(hardwareType) !== -1) {
            ...
            if (nic) {
              ...
              originalValue?.l3NetworkUuid &&
              !originalValue.l3Network?.uuid
            ) {
              // 普通用户无权限查看名称展示UUID
              info = originalValue.l3NetworkUuid;
            } else {
              info = intl.formatMessage({
        ...
        const showError = (hardwareType: IHardwareType) => {
          ...
          if (hardwareType === IHardwareType.Disk) {
            ...
            if (
              (createType === "new" || createType === "image") &&
              !diskSize?.number
            ) {
              return true;
            }
          ...
          if (hardwareType === IHardwareType.Netcard) {
            ...
            if (
              !nic &&
              (!originalValue?.l3NetworkUuid || !!originalValue.l3Network?.uuid)
            ) {
              return true;
            }
            ...
            <div className={styles.info}>
              ...
              <div
                className={cls(styles.delete, {
                  [styles.disableRemove]: !!disableRemove,
                })}
              >
                {((titleIndex !== "0" && setRemoveItemKey) ||
                  ([
                    IHardwareType.GPU,
                    IHardwareType.PCIe,
                    IHardwareType.USB,
                    ...
                    setRemoveItemKey)) && (
                  <Tooltip
                    title={disableRemove ? disableRemoveTooltip : undefined}
                  >
                    <Icon
                      size={16}
                      type="trash"
