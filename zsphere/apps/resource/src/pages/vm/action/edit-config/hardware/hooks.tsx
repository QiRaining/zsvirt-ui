...
const useQueryReleatedResource = (
  ...
  source?: { tpmList?: Array<{ uuid?: string; vmInstanceUuid?: string }> },
) => {
  const { data: volumeData, loading: volumeLoading } = useQuery<{
    volumeList: VolumeList;
  }>(volumeListForEditVM, {
    variables: {
      type: VolumeQueryType.GetVolumeByVMAndHostForEditVM,
      extraConditions: [
  ...
  const { data: vmDnsData, loading: vmDnsLoading } = useQuery(queryVmDns, {
    variables: {
      conditions: [{ key: "vmInstanceUuid", op: Op.eq, value: vmUuid }],
    },
  });
  ...
  const result = useMemo(() => {
    ...
    const formatedLunData = lunDataList.map((t: any) => {
      const ref = t.scsiLunVmInstanceRefs?.find(
        (item: any) => item.vmInstanceUuid === vmUuid,
      );
      return {
        ...t,
        lastAttachDate: ref?.lastOpDate,
    ...
    return {
      ...
      vmDnsList: vmDnsData?.queryVmDns?.list ?? [],
