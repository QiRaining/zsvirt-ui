...
import { verifyVmMigrateChangePrimaryStorage } from "@zstack/virtualization-resource/src/pages/vm/action/validators";
...
const FOOTER_STYLE = { textAlign: "right" } as const;
...
const migrateJudge = (vm: IVM, data: any): boolean => {
  ...
  // 共享块存储检查
  const isSharedBlockStorage = primaryStorage?.type === "SharedBlock";
  if (isSharedBlockStorage) {
    const hasNonSharedBlockVolume = allVolumes
      ?.filter((it) => it.type !== "Memory")
      ?.some((it) => it?.primaryStorage?.type !== "SharedBlock");
    if (hasNonSharedBlockVolume) {
      return false;
    }
  }
  ...
  // 通用限制检查
  const restrictions = {
    hasAttachedIso: vmCdRoms?.some((it) => it.isoUuid),
    ...
    hasVFNic: vmNics?.some((item) => item.type === "VF"),
  };

  // 根据虚拟机状态判断
  switch (state) {
    case VmInstanceState.Running:
    case VmInstanceState.Paused: {
      ...
      // 运行状态额外检查VF网卡
      const runningRestriction =
        state === VmInstanceState.Running &&
        restrictions.hasVFNic &&
        vm?.toolsState !== GuestToolsState.Installed;

      return !(baseRestrictions || runningRestriction);
    }
...
const BatchChangeStorage: React.FC<IActionWrapperProps<IVM>> = ({
  refetch,
  visible,
  setVisible,
  selectedList,
  ...
  const [migrateableVisible, setMigrateableVisible] = useState<boolean>(false);
  //状态&行为检测弹窗
  const [batchSelectedList, setBatchSelectedList] = useState<any[]>([]);
  ...
  const modalConfirmContent = useMemo(() => {
    let content = intl.formatMessage({
      id: "instance.modal.migrate.data.storage.action.judge.content",
      defaultMessage: "All selected virtual machines are unable to change data storage.",
    });
  ...
  const canMigrateList = useMemo(() => {
    ...
    return selectedList.filter(
      (t) => verifyVmMigrateChangePrimaryStorage(t) && migrateJudge(t, data),
    );
  }, [selectedList, data]);
  ...
  useEffect(() => {
    if (
      ...
      //状态校验
      if (cannotMigrateList && cannotMigrateList.length !== 0) {
    ...
    visible,
    canMigrateList,
    data?.storageMigrateVmInstancesDepends,
    data,
  ]);
  ...
  if (data && visible) {
    ...
    if (
      ...
      return (
        <DialogP3
          title={intl.formatMessage({
            id: "vm.migrate.modal.title.cannot.change.data.storage",
            defaultMessage: "Cannot Change Data Storage",
          })}
...
export default BatchChangeStorage;
