...
const HardwareInfo: React.FC<IProps> = ({
  setOriginConfig,
  form,
  source,
  vmTemplate,
    ...
    if (relatedData) {
      ...
      volumeList?.forEach((volume, index) => {
        if (volume.diskType === "volume") {
          let busType;
          if (volume.type === "Root") {
            busType = source?.systemTag?.vmDriver ? "virtio" : "ide";
          } else {
  ...
  const addHardwareItem = (hardwareType: IHardwareType) => {
    ...
    if (hardwareType === IHardwareType.Netcard) {
      ...
      initValue = {
        ...
        [`nicType-${addedNum}`]:
          source?.guestOsType === "Linux" ? "virtio" : "e1000",
          ...
          <div className={styles.left}>
            ...
            >
              {({ getFieldsValue }) => {
                ...
                const hasVgpuDevice = _.keys(values)
                  .filter((key) => key.indexOf("gpuDeviceType-") > -1)
                  ?.some((key) => values[key] === "vgpu");
                return (
                  <Dropdown
                    placement="bottomRight"
                    overlay={getHardwareMenu(hasVgpuDevice)}
                    trigger={["click"]}
                  >
