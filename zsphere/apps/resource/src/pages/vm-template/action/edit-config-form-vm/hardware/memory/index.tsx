...
import { Form, InputUnit, Select } from "@zstack/zsphere-components";
...
const allCanDisableItems = ["memoryResourceLevel", "memHotPlug"];
    ...
    <div className={styles.content}>
      ...
      >
        <InputUnit
          disabled={
            disabledConfig?.memoryDisabled ||
            disableItems.includes("memorySize")
          }
...
2. We recommend that you set Memory Resource Priority to High for vital VMs.
      ...
      >
        <Select
          ...
          width={200}
          disabled={disableItems.includes("memoryResourceLevel")}
        >
          <Option value="Normal">
            {intl.formatMessage({
              id: "normal",
      ...
      <Item noStyle shouldUpdate={(pre, cur) => pre?.hotPlug !== cur?.hotPlug}>
        {() => {
...
Default: enabled. Specify whether to allow online modification of VM's memory.`,
            ...
            >
              <Switch
                disabled={disableItems.includes("memHotPlug")}
                checked={hotPlug}
              />
            </Item>
          );
