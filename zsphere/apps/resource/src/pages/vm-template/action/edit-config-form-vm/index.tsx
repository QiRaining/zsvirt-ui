...
import { formatStorageToObj, formatResourceName } from "@zstack/zsphere-utils";
...
const editVmTemplateConfig = gql`
  mutation updateVmTemplate($input: EditTemplatedVMConfigInput!) {
    editVmTemplateConfig(input: $input) {
      actionId
    }
  }
`;
...
const EditInstance: React.FC<IActionWrapperProps<IVM>> = ({
  ...
  const {
    ...
    resetConfig,
    getDisabledConfig,
  } = useTransformPayload(selectedList?.[0]);
  ...
  const transformPayload = (value: any) => {
    ...
    // const nicValues = _.pickBy(value, (_value, key) => nicProps.indexOf(key.split('-')[0]) > -1)
    // const originNicValues = _.pickBy(
    //   originConfig,
    //   (_value, key) => nicProps.indexOf(key.split('-')[0]) > -1
    // )
    // nicTransform(nicValues, originNicValues, selectedList?.[0])

    // const cdromProps = ['cdRomList', 'removecdrom', 'cdRomListUuid']
    // const cdromValues = _.pickBy(value, (_value, key) => cdromProps.indexOf(key.split('-')[0]) > -1)
    ...
    // const usbProps = ['usbDiviceType', 'usbDivice', 'usbDiviceUuid', 'removeusb']
    // const usbValues = _.pickBy(value, (_value, key) => usbProps.indexOf(key.split('-')[0]) > -1)
    ...
    // const gpuProps = ['gpuDeviceType', 'gpuDevice', 'vgpuDevice', 'gpuDeviceUuid', 'removegpu']
    // const gpuValues = _.pickBy(value, (_value, key) => gpuProps.indexOf(key.split('-')[0]) > -1)
    ...
    try {
      ...
      doAction({
        ...
        name: intl.formatMessage({
          id: "templated.vm.modal.title.edit.config",
          defaultMessage: "Modify VM Template Configuration",
        }),
        total: 1,
        onFinish: () => {
          form.resetFields();
    ...
    <>
      <ConfigProvider {...disableCondigProps}>
        ...
        >
          ...
          >
            <BasicCard form={form} source={selectedList[0]} />
            <ZSVForm.Tabs
              tabs={[
                {
                  key: "hardware.info",
                      ...
                      {({ getFieldsError }) => {
                        return (
                          getFieldsError().find(
                            (item) =>
                              !!item.errors.length &&
                              !/(^name|^ipv4-|^description)/.test(
                                item.name[0].toString(),
                              ),
                          ) && (
                            <Icon
                              className={style.titleAlertIcon} type="alert-triangle-fill"
                            />
                          )
                        );
                      }}
        ...
        alertMessage={intl.formatMessage(
          {
            ...
            defaultMessage:
              '修改了{allChangeKeysNum} 项设置，以下 {needRebotKeysNum} 项需要重启虚拟机后方可生效。'
          },
          {
            allChangeKeysNum: changeKeys.length,
            needRebotKeysNum: _.sumBy(confirmList, item => (item.value as string[])?.length)
          }
        )}
