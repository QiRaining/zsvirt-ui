...
import { SystemTagActionType, VGpuType } from "@zstack/zsphere-types";
...
export const useTransformPayload = (vm: IVM) => {
  const intl = useIntl();
  ...
  const cpuBindListByVCpuTransform = (
    value: any,
    uuid: string,
    values: any,
  ) => {
  ...
  const tagTransform = (value: any) => {
    const originTagUuids = vm?.tag?.map((tag: any) => tag.uuid) ?? [];
    const currentTagUuids = value?.map((tag: any) => tag.uuid) ?? [];
    const removeTagUuids = _.difference(originTagUuids, currentTagUuids);
    const addTagUuids = _.difference(currentTagUuids, originTagUuids);
  ...
  const updateKey2Payload: { [key: string]: any } = {
    name: commonTransform("name", "updateTemplatedVMPayload"),
    description: commonTransform("description", "updateTemplatedVMPayload"),
    tags: tagTransform,
    guest: commonTransform("platform", "updateTemplatedVMPayload"),
    ...
    os: commonTransform("guestOsType", "updateTemplatedVMPayload"),
    // cpu
    totalCoreNum: commonTransform("cpuNum", "updateTemplatedVMPayload"),
    ...
    motherboardType: resourceConfigTransform("vmMachineType", "vm"),
