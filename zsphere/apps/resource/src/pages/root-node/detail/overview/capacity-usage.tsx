...
const divFlexAlignCenterStyle = {
  display: "flex",
  alignItems: "center",
} as const;

const CapacityUsage: FC<IProps> = React.memo(
  ({ onCollapseChange, collapsed = false }) => {
    ...
    const handleRefresh = useCallback(() => {
      cpuMemoryRefetch();
      storageRefetch();
    }, [cpuMemoryRefetch, storageRefetch]);
    ...
    return (
      <DraggableCard
        title={
          <div
            className={`flex items-center gap-1 ${styles.capacityContainer}`}
          >
            <span>
              {intl.formatMessage({
                id: "storage.info",
            ...
            <InfoPopover
              content={
                <ReactMarkdown>
                  {intl.formatMessage({
                    id: "root.storage.info.tooltip",
                    defaultMessage:
                      "# 容量信息\n" +
                      "展示平台所有数据中心下物理 CPU 、物理内存和物理存储资源总量以及使用情况。",
                  })}
                </ReactMarkdown>
              }
            />
        ...
        extra={
          <div className="flex items-center gap-3">
            ...
            <div style={divFlexAlignCenterStyle}>
              <Tooltip
...
CapacityUsage.displayName = "CapacityUsage";
