...
import {
  createSnapshotStrategyExecutionLogToolbarSchema,
  type SnapshotStrategyExecutionLogToolbarValues,
} from "./schema";
...
export default function Toolbar({ setCondition }: IProps) {
  const intl = useIntl();
  const form = useForm<SnapshotStrategyExecutionLogToolbarValues>({
    resolver: zodResolver(createSnapshotStrategyExecutionLogToolbarSchema()),
  ...
  const handleDurationChange = (
    value: SnapshotStrategyExecutionLogToolbarValues["duration"],
  ) => {
    const now = dayjs();
    if (value === "-1") {
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    ...
                    handleDurationChange(
                      value as SnapshotStrategyExecutionLogToolbarValues["duration"],
                    );
                  }}
                />
              </FormControl>
