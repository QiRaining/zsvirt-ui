...
import {
  ...
  type ManagementServerLogLastModifiedFormValues,
} from "./schema";
...
const ManagementServerLogLastModified: React.FC<IProps> = ({
  visible,
  setVisible,
  currItem,
  ok,
  ...
  const intl = useIntl();
  const defaultValues =
    useMemo<ManagementServerLogLastModifiedFormValues>(() => {
      const value = currItem?.formItem?.value;

      return {
        setting: value !== "-1",
        retentionTime: value && value !== "-1" ? Number(value) : "",
      };
    }, [currItem?.formItem?.value]);
  const formSchema = useMemo(
    () => createManagementServerLogLastModifiedSchema(intl),
    [intl],
  );
  const form = useForm<ManagementServerLogLastModifiedFormValues>({
  ...
  const onOk = (values: ManagementServerLogLastModifiedFormValues) => {
    return ok(
      [
        {
...
export default ManagementServerLogLastModified;
