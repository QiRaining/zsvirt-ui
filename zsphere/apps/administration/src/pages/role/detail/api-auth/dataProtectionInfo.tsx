...
interface IDataProtectionInfoProps {
  detail?: IZsvRole;
...
const DataProtectionInfo: FC<IDataProtectionInfoProps> = ({
  detail,
  onCollapseChange,
  collapsed,
  handleViewClick,
  ...
  const dataProtection = useAPIAuthPrivilegeListByKey({
    intl,
    targetKey: ITargetKey.dataProtection,
    detail,
  });
  const dataProtectionMemo = useMemo(
    () => dataProtection?.children,
    [dataProtection],
  );

  const list: ListItem[] = useMemo(() => {
    return dataProtectionMemo?.map((item: any) => {
      return {
        label: item.name,
    ...
    <>
      ...
      {editConfigVisible && (
        <EditResourceAuth
          ...
          selectedList={dataProtectionMemo}
          source={detail}
          title={title}
        />
      )}
