...
const CHECK_HOSTNAME_REPEAT = gql`
  mutation checkHostnameRepeat($input: CheckHostnameRepeatParam!) {
    checkHostnameRepeat(input: $input) {
      repeat
    }
  }
`;
...
export const useValidator = () => {
  ...
  const validReservedCapacity = async (
    ...
    const isInteger = _.isInteger(_number);
    ...
    if (validMaxSettingCapacity.isValid) {
      ...
      if (isInteger && isInRange && isUnderMax) {
        return;
      }
