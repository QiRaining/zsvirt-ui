...
interface IAutoWayProps {
  onSelectedMonListChange: (backupStorages: IBackupStorage[]) => void;
}

const AutoWay: FC<IAutoWayProps> = ({ onSelectedMonListChange }) => {
  const intl = useIntl();
  ...
  useEffect(() => {
    ...
    if (
      ...
      // 通知父组件选中的备份存储
      onSelectedMonListChange(
        initialBs.filter((bs: IBackupStorage) => bs.selected),
      );
  ...
  const handleSelectionChange = (newSelectedRowKeys: React.Key[]) => {
    ...
    // 通知父组件选中的备份存储
    onSelectedMonListChange(updatedBs.filter((bs) => bs.selected));
  };
