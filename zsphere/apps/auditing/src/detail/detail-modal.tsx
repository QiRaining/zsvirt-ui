...
const Share: React.FC<AuditDetailProps> = ({
  row = {},
  view = "main.resource",
  visible = false,
  setVisible = () => {},
  ...
  const handleOpenChange = (open: boolean | ((prev: boolean) => boolean)) => {
    setVisible(typeof open === "function" ? open(visible) : open);
  };

  return (
    <Drawer open={visible} setOpen={handleOpenChange} style={{ width: 600 }}>
      <DrawerHeader onClose={() => setVisible(false)}>
        {intl.formatMessage({ id: "event", defaultMessage: "Event" })}
      </DrawerHeader>
      <DrawerBody className={style.detailContainer}>
        <DraggableCard
        ...
        <div className={style.title}>
          {intl.formatMessage({ id: "apiRequest", defaultMessage: "API Request" })}
        </div>
        <DraggableCard
          className={style.card}
          title={intl.formatMessage({
        ...
        <div className={style.title}>
          {intl.formatMessage({ id: "apiResponse", defaultMessage: "API Response" })}
        </div>
        <DraggableCard
          className={style.card}
          title={intl.formatMessage({
