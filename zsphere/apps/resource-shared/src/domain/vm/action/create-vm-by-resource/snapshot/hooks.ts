...
import { compact, includes, replace } from "lodash-es";

import { CPU_COMPAT_OS_LIST } from "../constant";
import { getSystemTagsFromNoIPAMInput } from "../hooks";
import {
  SetDiskQosType,
  bandWidthUnitList,
...
export const transformParams = (
  ...
  const payload: any = {
    volumeSnapshotGroupUuid: realSource?.group?.uuid ?? realSource?.uuid, //快照 uuid
    name,
  ...
  if (motherboardType) {
    payload.motherboardType = motherboardType;
  }
  // 处理TPM配置方式
    ...
    vmNicConfigListWithData.forEach((t: any) => {
      ...
      if (t.staticIpv6) {
        payload.systemTags.push(
          `staticIp::${t.l3NetworkUuid}::${replace(t.staticIpv6, "::", "--")}`,
        );
      }
