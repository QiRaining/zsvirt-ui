...
const FeiShuContent: React.FC<IProps> = ({
  ...
  React.useEffect(() => {
    ...
    if (init) {
      formInstance.setFieldsValue({
        zwatchSNSTextTemplate: {
          feishu: get(init, ["zwatchSNSTextTemplate", "feishu"]),
        },
      });
  ...
  return (
    <>
      <Item
        ...
        required
        name={["zwatchSNSTextTemplate", "feishu", "subject"]}
        icon="info"
        iconTooltip={
          <ReactMarkdown>
            {intl.formatMessage({
...
When an alarm is triggered, the platform sends alarm messages to the selected endpoint. You can customize the text content of the alarm messages as needed.`,
        ...
        name={["zwatchSNSTextTemplate", "feishu", "template"]}
        rules={[isRequired()]}
      >
        <TextArea
          rows={15}
      ...
      {isResourceAlarm ? (
        <>
          <Item
            ...
            required
            name={["zwatchSNSTextTemplate", "feishu", "recoverySubject"]}
            rules={[isRequired()]}
            icon="info"
            iconTooltip={
              <ReactMarkdown>
...
When a monitored resource recovers from an alarm status, the platform sends alarm recovery messages to the selected endpoint. You can customize the text content of the recovery messages as needed.`,
            ...
            name={["zwatchSNSTextTemplate", "feishu", "recoveryTemplate"]}
            rules={[isRequired()]}
          >
            <TextArea
              rows={15}
