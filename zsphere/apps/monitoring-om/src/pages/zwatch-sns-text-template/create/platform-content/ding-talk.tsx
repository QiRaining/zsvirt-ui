...
const DingTalkContent: React.FC<IProps> = ({
  ...
  React.useEffect(() => {
    ...
    if (init) {
      formInstance.setFieldsValue({
        zwatchSNSTextTemplate: {
          dingTalk: get(init, ["zwatchSNSTextTemplate", "dingTalk"]),
        },
      });
  ...
  return (
    <>
      <Item
        ...
        required
        name={["zwatchSNSTextTemplate", "dingTalk", "subject"]}
        icon="info"
        iconTooltip={
          <ReactMarkdown>
            {intl.formatMessage({
...
When an alarm is triggered, the platform sends alarm messages to the selected endpoint. You can customize the title of the alarm messages as needed.`,
      ...
      <Item
        ...
        name={["zwatchSNSTextTemplate", "dingTalk", "template"]}
        icon="info"
        iconTooltip={
          <ReactMarkdown>
            {intl.formatMessage({
      ...
      {isResourceAlarm ? (
        <>
          <Item
            ...
            required
            name={["zwatchSNSTextTemplate", "dingTalk", "recoverySubject"]}
            icon="info"
            iconTooltip={
              <ReactMarkdown>
                {intl.formatMessage({
...
When a monitored resource recovers from an alarm status, the platform sends alarm recovery messages to the selected endpoint. You can customize the text content of the recovery messages as needed.`,
            ...
            name={["zwatchSNSTextTemplate", "dingTalk", "recoveryTemplate"]}
            rules={[isRequired()]}
          >
            <TextArea
              rows={15}
...
export default DingTalkContent;
